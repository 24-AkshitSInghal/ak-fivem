if Config.BinocularsEnabled then
    ...
    RegisterCommand("binoculars", function()
        local hasBinoculars = exports.coca_inventory:HasItemInInventory('binoculars')
        if hasBinoculars then
            UseBinocular()
        else
            drawNativeNotification("Don't have binoculars")
        end
    end)
    TriggerEvent('chat:addSuggestion', '/binoculars', 'Take Out Binoculars', {})


    local fov_max = 70.0
    local fov_min = 10.0 -- max zoom level (smaller fov is more zoom)
    local zoomspeed = 17.0 -- camera zoom speed
