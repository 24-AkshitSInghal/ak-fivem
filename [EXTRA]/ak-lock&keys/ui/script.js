...
document.addEventListener("keydown", function(ev) {
    ...
    let valid_keys = ['1', '2', '3', '4', 'Escape', 'Tab'];
    if (!valid_keys.includes(key_pressed)) {
        return; 
    }
    
    if (key_pressed === 'Escape' || key_pressed === 'Tab') {
        wrong(); 
        return;
    }
...
function endGame(status) {
    $('#canvas').hide();
    let u = status ? "success" : "fail";
    $.post(`https://ak-lock/${u}`, JSON.stringify({}));
    streak = 0;
    needed = 4;
}

window.addEventListener("message", (event) => {
    if (event.data.type == "startLockpick") {
