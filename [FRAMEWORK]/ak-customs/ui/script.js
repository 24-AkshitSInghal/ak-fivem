...
document.onkeyup = function(data) {
    ...
    if (key == "ArrowRight") {
        ...
        if (selected == "main") {
            if (mainMenus[selectedId].label == "Paint Booth") {
                openPaintMenu()
                $.post('https://ak-customs/cursorState', JSON.stringify({
                    cursor: true,
                }));
            }else if (mainMenus[selectedId].label == "Wheel Fitment") {
                $.post('https://ak-customs/getWheelFitment', JSON.stringify({}), function(fitmentData){
                    wheels = fitmentData;
                    openFitment()
                });
                $.post('https://ak-customs/cursorState', JSON.stringify({
            ...
            if (attached === "false" || !attached) {
                $.post('https://ak-customs/buyComponent', JSON.stringify({
        ...
        if (inColorMenu) {
            ...
            if (selected === "altmenu") {
                ...
                $.post('https://ak-customs/close', JSON.stringify({}));
    ...
    if (key == "Backspace" || key == "Enter" || key == "ArrowLeft" || key == "ArrowRight") {
        ...
        $.post('https://ak-customs/surfItem', JSON.stringify({
...
$('body').mousemove(function() { 
    if (activeMenuName == "altmenu") {
        $.post('https://ak-customs/disableCamera', JSON.stringify({}));
    }
});

colorPicker.on('color:change', function(color) {
    $.post('https://ak-customs/changeColor', JSON.stringify({
        type: {
...
const buildMenu = (menu, objectId, component) => {
    ...
    $.post('https://ak-customs/rotateCamera', JSON.stringify({
        menu: activeMenuName,
        objectId: objectId,
        component: component
    }));
...
$(document).on('click', '#arrow', function(){
    if (!cooldown) {
        ...
        if (type != "fitment") {
            ...
            $.post('https://ak-customs/changeFitment', JSON.stringify({
                type: fitmentName,
...
$(document).on('click', '#buyColor', function(){
    if (!cooldown) {
        cooldown = true;
        $.post('https://ak-customs/buyColor', JSON.stringify({
            type: {
                first: firstSelectorItems[firstIndex],
                second: secondSelectorItems[secondIndex]
            },
...
$(document).on('click', '#buyFitment', function(){
    if (!cooldown) {
        cooldown = true;
        $.post('https://ak-customs/applyFitment', JSON.stringify({}), function() {
            cooldown = false;
        });
    }
})
...
function changeLogoColor(start) {
    const $logo = $('.mechanic-logo');
    
    if (start) {
        let colorIndex = 0;

        colorChangeInterval = setInterval(() => {
            $logo.css('background-color', neonColors[colorIndex]);
...
window.addEventListener('message', function (event) {
    ...
    switch (e.type) {
        case "OPEN_MENU":
            isIllegalMechanic = e.isIllegalMechanic
               
            if(isIllegalMechanic){
                 $('#legal').hide()
                $('#illigal').show();
            }else{
                 $('#legal').show()
