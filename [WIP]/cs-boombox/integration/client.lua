...
RegisterCommand('boombox', function()
    ...
    if uiAccessible then
        ...
        print("Number of objects in game pool:", #objects)

        for i = 1, #objects do -- Lua tables start from 1
            local object = objects[i]

            if DoesEntityExist(object) and not HasObjectBeenBroken(object) then
                local model = GetEntityModel(object)
                print(string.format("Object %d - Model: %d", i, model))

                if model == GetHashKey('prop_boombox_01') and NetworkGetEntityIsNetworked(object) then
                    local distance = #(GetEntityCoords(object) - playerCoords)
                    print(string.format("Object %d - Distance: %f", i, distance))

                    if distance <= 2.0 and ((not closest.handle) or distance < closest.distance) then
                        closest.handle = object
                        closest.model = 'prop_boombox_01'
                        closest.distance = distance
                        closest.uniqueId = tostring(NetworkGetNetworkIdFromEntity(object))
                        print(string.format("New closest object found - Handle: %d, Distance: %f, Unique ID: %s",
                            closest.handle, closest.distance, closest.uniqueId))
                    end
                end
            end
        end
    ...
    if closest.handle then
        ...
        print("Triggering server event for closest object:", closest.uniqueId, closest.model)
        TriggerServerEvent('cs-boombox:integration:toggleControllerInterface', closest.uniqueId, closest.model)
    else
        lastAccessedObject = nil
        print("No boombox object found within range")
    end
end, false)
